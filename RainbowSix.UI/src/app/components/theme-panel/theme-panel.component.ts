...
export class ThemePanelComponent implements OnInit {
	...
	@Output() appTemplateModeChanged = new EventEmitter<boolean>();
	...
	appTemplateModeEnabledCheckbox: boolean = false;
	...
	ngOnInit() {
		...
		if (localStorage) {
			...
			if (localStorage['appTemplateMode'] && localStorage['appTemplateMode'] === 'true') {
				this.appTemplateModeEnabledCheckbox = true;
				this.appTemplateModeChanged.emit(true);
			}
		}
	}
	
	toggleTemplateMode(e){
		this.appSettings.appTemplateMode = e.srcElement.checked;
		this.appTemplateModeChanged.emit(true);
		if (localStorage) {
			localStorage['appTemplateMode'] = e.srcElement.checked;
		}
